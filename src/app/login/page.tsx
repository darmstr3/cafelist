...
// Toggle this once Google OAuth is enabled in the Supabase dashboard. Until
// then, the button silently errors because Supabase has no Google provider
// configured — looks broken to users. Hide the button rather than ship a
// dead control. Set NEXT_PUBLIC_GOOGLE_AUTH_ENABLED=true on Vercel when ready.
      ...
      <div className="max-w-md mx-auto px-4 sm:px-6 py-16">
        {status === 'sent' ? (
          ...
          <>
            ...
            {GOOGLE_AUTH_ENABLED && (
              <>
                {/* Google sign-in — only renders when the Supabase Google
                    provider is configured. Set NEXT_PUBLIC_GOOGLE_AUTH_ENABLED
                    on Vercel to flip it on. */}
                <button
